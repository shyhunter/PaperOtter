...
Most of PaperOtter works with nothing extra to install. Two tools are bundled or
...
PaperOtter** and needs neither of the programs below. This page previously said
...
`soffice` is on your `PATH`. Without it, PaperOtter names the missing program
...
provide, so PaperOtter hides them where they cannot work rather than offering
...
HEIC is limited by patent licensing rather than effort: PaperOtter never ships its
