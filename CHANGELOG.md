...
- **Papercut is now PaperOtter.** New name, new logo, and a redesigned interface.
  ...
  a previous install's preferences are not carried over.
...
- **Nine languages.** English, German, Turkish, French, Spanish, Italian, Dutch,
  Polish and Portuguese, picked from the top bar and remembered. PaperOtter
  guesses from the operating system on first launch. Plural forms are handled
  properly, Polish has three, and several languages have categories English does
  not. *All nine are AI-assisted; see the note at the end.*
...
- **Saved settings.** Name the settings you use often and pick them up again
  ...
  PaperOtter claiming to know which office you are dealing with.
- **Compression works out of the box** on macOS, Windows and Linux, with
  nothing to install.
...
These are the changes that stop PaperOtter damaging something of yours.

- **Save no longer risks your original.** Writing over a file is now atomic:
  PaperOtter writes a complete copy alongside it and swaps it in, so an
...
- **Password-protected PDFs are refused at the door.** Eleven tools used to open
  one and show an empty document; they now say what is wrong instead.
- **Redaction boxes are opaque.** A placed box was see-through, so redacted text
  could still be read.
- **Page rotation survives compression.**
...
- **Reading scans and HEIC input are macOS-only.** Both rely on what the
  operating system provides. PaperOtter hides them elsewhere rather than offering
...
- **Converting to EPUB or MOBI needs [Calibre](https://calibre-ebook.com/)**, and
  ...
  other eighteen tools need nothing installed.
